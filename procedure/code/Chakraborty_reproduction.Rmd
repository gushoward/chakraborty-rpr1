...
# replace NA with 0
acs_covid <- acs_covid |> replace.na(list(bpov_pct = 0,
apov_pct = 0))
```
...
Given the  differences across the results and some of the data, our analysis diverged from the original study in a number of ways. Despite reproducing the satscan package and the author's other data files
For example, the GEE models that were used in the original Chakraborty study  

The satscan clusters, in certain cases, misrepresented a cluster, assigning it to a larger region when the original study may have perscribed the Covid risk to that of only a singular county.
...
```{r load-table2}
table2 <- read.csv(here("data", "raw", "public", "table2.csv"))
table2 <- table2 %>%
  arrange(row_i_first) #%>%
